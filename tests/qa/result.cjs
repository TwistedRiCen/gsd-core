...
const KIND = Object.freeze({
  ...
  /** exit 1, stdout parsed as a JSON object with no `error` key, and no stderr envelope — a declared FAIL verdict carrying its payload (ADR-3889 §1, #4686). NOT an invocation error. */
...
function classify(raw, io = {}) {
  ...
  if (raw.exitCode === 1) {
    ...
    // populated. Deliberately NARROW: only a JSON object with no `error` key
    // qualifies (an `{error:...}` payload is the soft-failure idiom, a scalar
    // is not a verdict), and the stderr envelope above still wins.
